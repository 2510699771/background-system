<template>
  <div>
    <div style="width:30%;margin-bottom:20px">
      <el-input placeholder="请输入内容" v-model="query" @input="theinput" class="input-with-select">
...
export default {
  ...
  methods: {
    ...userActions(["getOrders", "getKuaidi"]),
    theinput(e) {
      if (e) {
        console.log(e);
      },
      queryinfo(){this.getorder();}
    },
    //每页显示条数
    handleSizeChange(e) {
      this.pagesize = e;
    ...
    //点击确定
    confirm(e) {
      console.log(e.value.goods_name);
    },
    //点击空白关闭
    present() {
      this.Visible = false;
    ...
    location() {
      ...
      this.title = "物流信息";
      this.Visible1 = true;
      console.log(this.Dialogfrom, "6464646F");
    },
    //获得总数据
    getorder() {
      this.getOrders({
  ...
  mounted() {
    this.getorder();
    this.Dialogfrom1[0].attrs.options = area;
  },
  watch: {
    kuaidi() {
  ...
  computed: {
    //数据展开
    ...userState(["orderList", "total", "kuaidi"]),
  },
};
</script>
