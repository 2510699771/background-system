          ...
          >
            <div style="width:20vw">
              <el-tag
                closable
                type="primary"
                @close="guanbi(item,scope.scope.row,scope.scope.$index)"
              ...
              >
                <div style="width:20vw">
                  <el-tag
                    closable
                    type="success"
                    @close="guanbi(ite,scope.scope.row,scope.scope.$index)"
                  >{{ite.authName}}</el-tag>
                  <i class="el-icon-caret-right"></i>
                </div>
                  ...
                  <div v-for="it in ite.children" :key="it.id" style="margin:10px">
                    <div>
                      <el-tag
                        closable
                        type="warning"
                        @close="guanbi(it,scope.scope.row,scope.scope.$index)"
                      >{{it.authName}}</el-tag>
                    </div>
                  </div>
                </div>
  ...
  methods: {
    ...
    guanbi(a, b) {
      ...
      this.roleList.map((item, index) => {
        if (item.id === a.id) {
          this.roleList.spilce(index,1)
        }else if(item.id != a.id &&item.children){
          item.children.map((item1,index1)=>{
            if(item1.id===a.id){
              item.children.spilce(index1,1)
            }else if (item1.id!=a.id &&item1.children){
              item1.children.map(item2,index2_)
            }
          })
        }
      });
