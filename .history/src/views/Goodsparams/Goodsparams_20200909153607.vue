    ...
    <el-tabs v-model="activeName" @tab-click="handleClick">
      <el-tab-pane label="动态参数" name="first">
        ...
        >
          <template v-slot:expand="scope">
            <el-tag
              :key="index"
              v-for="(tag,index) in scope.scope.row.attr_vals"
              closable
      ...
      <el-tab-pane label="静态属性" name="second">
        ...
        >
          <template v-slot:expand="scope">
            <div
              v-if="scope.scope.row.attr_vals[0]!='' &&scope.scope.row.attr_vals.length>0"
              class="tag"
            >
              <el-tag
                :key="index"
...
export default {
  ...
  methods: {
    ...
    handleClose(tag, scope) {
      ...
      scope.scope.row.attr_vals.splice(
        scope.scope.row.attr_vals.indexOf(tag),
        1
      );
    },
    //点击添加标签按钮
    ...
    handleInputConfirm(scope) {
      ...
      if (inputValue) {
        scope.scope.row.attr_vals.push(inputValue);
      }
  ...
  mounted() {
    //  let a = [];
    //  this.$store.good.orieslist("setories",[]);
    //  this.$store.commit("setories", a);
    //  console.log(this.$store.state.good.commit, "88888");
    console.log(this.$store.good.commit("setories", 0));
    //层级选择数据
    this.categories({
      type: this.type,
